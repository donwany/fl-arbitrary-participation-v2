...
directory1 = '/home/ts75080/Desktop/fl-arbitrary-participation/fashion_test_results/05'
directory2 = '/home/ts75080/Desktop/fl-arbitrary-participation/cifar_test_results/05'
...
files1 = ['fashion_wait_minibatch_alpha_0.5.csv',
          'fashion_wait_full_alpha_0.5.csv',
          'fashion_alg1_no_amplify_alpha_0.5.csv',
          'fashion_alg1_amplify_alpha_0.5.csv'
          ]
...
legends1 = [r'wait-minibatch', r'wait-full',
            r'Alg. 1 without amplification', r'Alg. 1 with amplification']
legends2 = [r'wait-minibatch', r'wait-full',
            r'Alg. 1 without amplification', r'Alg. 1 with amplification']
...
for f_index in range(len(files1)):
    ...
    if moving_average_window > 1:
        ...
        entries_accuracy = moving_average(
            entries_accuracy, moving_average_window)

    keys_list = list(entries_loss.keys())
    amp_factor = float(keys_list[-1] + moving_average_window *
                       (keys_list[-1] - keys_list[-2])) / float(keys_list[-1])
    # Stretch x-axis for moving average window
    rounds = np.array(keys_list, dtype=float) * \
        amp_factor / float(iterations_per_round)
    ...
    ax1.plot(rounds[start_index:],
             entries_loss_mean[start_index:], label=legends1[f_index])
    ax1.fill_between(rounds[start_index:],
                     entries_loss_mean[start_index:] -
                     entries_loss_std[start_index:],
                     entries_loss_mean[start_index:] +
                     entries_loss_std[start_index:],
                     alpha=0.1)

    ax2.plot(rounds[start_index:],
             entries_accuracy_mean[start_index:], label=legends1[f_index])
    ax2.fill_between(rounds[start_index:],
                     entries_accuracy_mean[start_index:] -
                     entries_accuracy_std[start_index:],
                     entries_accuracy_mean[start_index:] +
                     entries_accuracy_std[start_index:],
                     alpha=0.1)
...
for f_index in range(len(files2)):
    ...
    ax3.plot(rounds[start_index:],
             entries_loss_mean[start_index:], label=legends2[f_index])
    ...
    ax4.plot(rounds[start_index:],
             entries_accuracy_mean[start_index:], label=legends2[f_index])
...
fig.savefig('alpha_0.5_results.png', dpi=200)
