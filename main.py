...
if __name__ == "__main__":
    ...
    for seed in simulations:
        ...
        # ----------------------------------------------------------------------
            # Initialize accumulators for weighted sum and total weights
            weighted_sum = None  # This accumulates the weighted updates from all nodes.
            total_weights = 0    # This keeps track of the sum of weights used for normalization.
        # ------------------------------------------------------------------------
            ...
            for n in node_subset:
                ...
                w_tmp = model.get_weight()  #  Local model weighs after training. deepcopy is already included here

                w_tmp -= w_global # Calculate the weight difference (update) compared to the global weights
                
            # -----------------------------------------------------------------------------------
               # Compute node weight (alpha_m * |s_m|), for simplicity assume alpha_m = 1
                alpha_m = alpha_m  # This can be configured as needed
                s_m = len(dict_users[n])  # Number of samples used by node n

                weight_factor = alpha_m * s_m

                if total_weights == 0:  # Initialize weighted sum with the first node's weighted update
                    weighted_sum = weight_factor * w_tmp    
                else:
                    weighted_sum += weight_factor * w_tmp # During the update accumulation, instead of simply summing the weight differences, we multiply each node's update by αm⋅∣sm∣.

                total_weights += weight_factor
            # ---------------------------------------------------------------------------------
            ...
            # Update global weights with the weighted average
            if total_weights > 0:
                w_global += torch.div(weighted_sum, torch.tensor(total_weights).to(device)).view(-1)
